...
- Render collections on the Modo Foco overlay, collapsed by default with name and member count; allow at most one expanded collection.
...
And the collapsed collection appears on the Modo Foco overlay with its member count.
...
3. Live Chrome: create **Casa**, add at least one main-list and one archived conversation, reload, expand the collection, and open both.
