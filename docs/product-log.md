...
- **[Em implementação] Coleções focadas fixas**
  - Até 5 coleções de 8 conversas, adicionadas explicitamente a partir da conversa focada e recolhidas por padrão no overlay. A extensão persiste em `chrome.storage.local` somente o nome escrito da coleção e os títulos escolhidos, reutiliza a navegação exata escondida e mantém recência, não lidas, previews e demais dados fora do modelo.
