...
- **[Em validação] Aguardar estabilização de correspondências na abertura exata**
  ...
  - Diagnóstico copiável adiciona `resultSamples`: até 12 amostras sanitizadas com índice da tentativa, contagens de linhas/títulos/correspondências e booleano de aceitação do termo. Nenhum nome, termo, conteúdo ou DOM entra no diagnóstico. A espera extra mínima é 150 ms, sem aumentar o limite de tentativas. 77 testes passam, incluindo duplicidade transitória/persistente, resultado único transitório, alvo substituído, cancelamento e sanitização. Falta repetir no Chrome a entrada que falhou; se persistir, usar as amostras antes de outra alteração de seletores.
