...
- **[Em validação] Suspender navegação focada sob modais e visualizadores nativos**
  ...
  - Inspeção direta estrutural confirmou Editar mensagem como `[role="dialog"][aria-modal="true"]` em uma camada fixa de viewport; imagem e vídeo reutilizam `[data-testid="media-viewer-modal"]`, fixo e de viewport inteiro. Nenhum texto/conteúdo foi lido ou exportado. A detecção usa somente esses dois sinais allowlisted, visibilidade real e exclusão de controles da extensão.
  - O observer DOM existente recalcula a visibilidade na abertura/fechamento. Shelf, Buscar, Adicionar à coleção e um seletor de coleção eventualmente aberto são suspensos pela classe `mwf-native-transient-open`; overlay inicial e dados em memória não mudam. Testes cobrem modal, media viewer, restauração sem mutar filhos, superfícies ocultas/da extensão, controles e integração com o observer. Navigator validou o shelf e Buscar. Adicionar à coleção permaneceu visível porque a regra normal `mwf-active:not(.mwf-overlay-open)` tinha maior especificidade e sobrescrevia o primeiro bloqueio, embora `pointer-events` já estivesse desativado. Override específico adicionado; inspeção direta com o media viewer aberto confirmou `display: none` nos dois botões e no seletor. Confirmação visual final do Navigator ainda pendente.
...
- **[Validado] Capturar aberturas a partir da camada Arquivados**
  ...
  - O limite agora aceita somente linhas com `cell-frame-title` que sejam `conversation-list-item`, `role=listitem` ou `role=row`, dentro de `#side` ou `archived-chatlist`. Isso inclui conversas arquivadas e exclui o botão Arquivadas e linhas fora das listas permitidas. Leitura do título, confirmação exata e tentativas não mudam. Testes cobrem lista principal, Arquivados, botão e falha fechada. Navigator recarregou e confirmou o comportamento no perfil separado e depois no perfil principal: “Ok, funcionou”. Chrome dedicado encerrado e porta 9222 verificada como fechada. Correção validada na `main`. O perfil separado permanece no disco e vinculado até o Navigator optar por desvincular/remover; não há acesso de depuração ativo.

- **[Diagnóstico concluído e removido] Captura intermitente dos recentes no modo completo**
  - Instrumentação temporária em memória distinguiu clique, confirmação, timeout, origem do título e mousedown/click sem exportar conteúdo. A inspeção direta revelou que os timeouts `frameTitle` eram do botão Arquivadas, enquanto as conversas arquivadas eram descartadas por estarem fora de `#side`.
  - Após a correção ser validada no perfil separado e no principal, foram removidos o link **Copiar diagnóstico dos recentes**, recorder, sonda de mousedown, metadados de comparação e testes exclusivos da investigação. Permanecem somente a captura funcional e o diagnóstico separado de falha na reabertura exata por busca.
