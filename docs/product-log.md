...
- **[Validação final no perfil principal] Capturar aberturas a partir da camada Arquivados**
  - Inspeção direta via perfil separado confirmou a causa: Arquivados permanece sobre a lista principal, em `[data-testid="archived-drawer"]`; suas conversas ficam em `[data-testid="archived-chatlist"]`, fora de `#side`, como linhas `[role="listitem"]`. Uma abertura controlada de conversa já lida mostrou mousedown/click na mesma linha, `frameChildTitle` estável e conectado, mas `side: false`. O handler exigia `closest("#side")` e descartava o evento antes da captura.
  - Os timeouts `frameTitle` anteriores pertenciam ao botão de navegação `[data-testid="chatlist-panel-archived-button"]`, que era confundido com conversa por aceitar qualquer `cell-frame-container`. Não eram evidência de falha de correspondência na conversa arquivada.
  - O limite agora aceita somente linhas com `cell-frame-title` que sejam `conversation-list-item`, `role=listitem` ou `role=row`, dentro de `#side` ou `archived-chatlist`. Isso inclui conversas arquivadas e exclui o botão Arquivadas e linhas fora das listas permitidas. Leitura do título, confirmação exata e tentativas não mudam. Testes cobrem lista principal, Arquivados, botão e falha fechada. Navigator recarregou e confirmou comportamento aparentemente correto no perfil separado. Chrome dedicado encerrado e porta 9222 verificada como fechada. Correção já aplicada na `main`; resta recarregar extensão/aba e confirmar no perfil principal antes de marcar validada. O perfil separado permanece no disco e vinculado até o Navigator optar por desvincular/remover; não há acesso de depuração ativo.

- **[Diagnóstico histórico] Captura intermitente dos recentes no modo completo**
  - Navigator relata conversas deliberadamente abertas no modo completo que às vezes não aparecem nos recentes ao voltar ao foco; repetir costuma funcionar. Relatou também aparição enquanto escrevia sem novo clique. A investigação anterior foi somente leitura; nenhuma mudança de código explica aquele episódio. Não atribuir ainda a timeout, evento perdido ou digitação.
  - Instrumentação somente em memória da aba: até 32 eventos estruturais e o último resultado terminal preservado separadamente. Observa mousedown/click/Enter, área side/main/other, reconhecimento de linha, modo, disponibilidade de título/cabeçalho, correspondência, tentativa, contador local de captura e duração relativa. Distingue checking/success/timeout/cancelled e adições à lista. Não exporta nomes, conteúdo, teclas digitadas (exceto enum Enter), DOM, identificadores de conversa nem timestamps absolutos. Não grava em storage/awareness.
  - Novo botão no overlay de foco: **Copiar diagnóstico dos recentes**, disponível mesmo sem recentes. Copia o estado atual mediante ação explícita. Ao ocorrer a falha ou aparição tardia, copiar antes de recarregar a aba; a recarga descarta a evidência.
  - Primeiro diagnóstico real: clique da lista reconhecido, título esperado e cabeçalho disponíveis, mas sem correspondência nas seis verificações; timeout após 2.460 ms. Isso descarta perda do clique/cancelamento naquela tentativa, mas não distingue demora de leitura divergente. Não prova a causa da aparição durante digitação.
  - Instrumentação ampliada: `rowSource`/`headerSource`, diferença atributo/texto, `headerChanged` entre observações e comparações diagnósticas `caseFoldedMatch`, `formatFoldedMatch` e `caseAndFormatFoldedMatch`. A comparação de formatação remove somente controles direcionais/zero-width/BOM de uma lista explícita; não é equivalência geral nem critério de aceitação. Nenhum texto entra no snapshot. `lastCapture` preserva até oito registros da última tentativa separadamente dos 32 eventos gerais, para não perder detalhes ao continuar usando a página.
  - Segunda evidência: mesma conversa às vezes funciona e às vezes falha, nomes visualmente iguais. Tentativa bem-sucedida usou frameChildTitle; a falha usou frameTitle. No timeout, nenhuma equivalência diagnóstica foi encontrada; o cabeçalho mudou somente na última observação, após outro clique fora das áreas reconhecidas. Não concluir que o contêiner está errado nem que houve atraso apenas com isso.
  - Nova sonda compara mousedown e click: `pointerObserved`, `pointerSameRow`, `pointerTitleAvailable`, `pointerTitleMatchesClick`, `pointerSource`, `pointerElapsedMs`. A observação bruta mantém uma referência de linha e um título somente até o clique, próximo mousedown ou expiração de cinco segundos; nunca os inclui em diagnóstico, storage ou logs. Evidência estrutural acompanha a tentativa em lastCapture. Captura ainda usa o título do click e a mesma confirmação estrita; não há abertura/adicionamento disparado por mousedown.
  - Navigator autorizou configurar acesso direto para reduzir o vai e volta. Chrome separado iniciado via `tools/start-debug-browser.ps1`, perfil local MirrorWhatsAppFocusDebug, CDP na porta 9222 verificado como loopback-only; conexão WebSocket confirmada por Browser.getVersion. Pendente: Navigator carregar a extensão estável e vincular o WhatsApp. Procedimento/encerramento em `docs/local-browser-debugging.md`. Inspeção estrutural, sem exportar mensagens/nomes, enviar mensagens ou alterar o perfil principal. Não expor porta à rede nem copiar o perfil real.
  - Não se alterou a política de captura: prazo de cinco retries de 300 ms, critérios de confirmação, cancelamento e handlers de abertura permanecem. Mousedown adicional é observação apenas. Testes verificam timeout/cancelamento, ausência de adição por observação mousedown/Enter no compositor sintético, limites e sanitização. Se o handler legado reconhecer incorretamente uma linha dentro da conversa, o diagnóstico agora poderá mostrar `zone: main` e uma captura posterior; isso ainda exige evidência real.
...
- **[Diagnóstico histórico] Ambiguidade persistente após distinguir capitalização**
  ...
  - O diagnóstico copiado agora inclui `matchStructure`, referente à última inspeção e apenas às correspondências exatas: número de alvos DOM distintos (`uniqueTargets`), origem do elemento escolhido (`frameChildTitle`, `frameTitle`, `autoSpanTitle`, `spanTitle`, `unknownSource`), diferença entre atributo/título selecionado e texto do elemento (`selectedTextDifferent`), diferença para texto completo do contêiner de título (`containerTextDifferent`) e ausência desse contêiner (`containerMissing`). Todos são contadores limitados a 1.000; nenhuma referência DOM, texto, nome, mensagem ou snapshot é exportado/persistido.
  - Identidade DOM não prova identidade da conversa: alvos distintos podem representar mensagens da mesma conversa. Contadores indicam onde investigar, não autorizam deduplicação ou abertura automática. Seletores, prioridade da leitura, comparação e critério de clique permanecem iguais. Próximo passo: reproduzir uma vez e examinar o novo bloco; sem nova hipótese de correção antes da evidência.
