...
- **[Em validação] Suspender navegação focada sob modais e visualizadores nativos**
  ...
  - O observer DOM existente recalcula a visibilidade na abertura/fechamento. Shelf, Buscar, Adicionar à coleção e um seletor de coleção eventualmente aberto são suspensos pela classe `mwf-native-transient-open`; overlay inicial e dados em memória não mudam. Testes cobrem modal, media viewer, restauração sem mutar filhos, superfícies ocultas/da extensão, controles e integração com o observer. Navigator já validou o shelf em editor, imagem e vídeo; validação dos dois botões ainda pendente após recarregar extensão/aba.
